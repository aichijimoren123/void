...
export const PastThreadsList = ({ className = "" }: { className?: string }) => {
	...
	if (!allThreads) {
		return (
			<div key="error" className="p-1">{`Error accessing chat history.`}</div>
		);
	}
	...
	const sortedThreadIds = Object.keys(allThreads ?? {})
		.sort((threadId1, threadId2) =>
			(allThreads[threadId1]?.lastModified ?? 0) >
				(allThreads[threadId2]?.lastModified ?? 0)
				? -1
				: 1
		)
	...
	const displayThreads = showAll
		? sortedThreadIds
		: sortedThreadIds.slice(0, numInitialThreads);

	return (
		<div
			className={`flex flex-col mb-2 gap-2 w-full text-nowrap text-void-fg-3 select-none relative ${className}`}
		>
					...
					if (!pastThread) {
						return (
							<div
								key={i}
								className="p-1"
							>{`Error accessing chat history.`}</div>
						);
					}
...
const PastThreadElement = ({
	...
	if (firstUserMsgIdx !== -1) {
		...
		firstMsg =
			(firsUsertMsgObj.role === "user" && firsUsertMsgObj.displayContent) || "";
	} else {
		firstMsg = '""';
	}
			...
			<div className="flex items-center justify-between gap-1">
				<span className="flex items-center gap-2 min-w-0 overflow-hidden">
					...
					{isRunning === "LLM" ||
						...
						isRunning === "idle" ? (
						<LoaderCircle
							className="animate-spin bg-void-stroke-1 flex-shrink-0 flex-grow-0"
							size={14}
						/>
					) : isRunning === "awaiting_user" ? (
						<MessageCircleQuestion
							className="bg-void-stroke-1 flex-shrink-0 flex-grow-0"
							size={14}
						/>
					) : null}
					{/* name */}
